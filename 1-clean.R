...
# ))
# 
# is.na(timeseries_data) %>% sum()
# 
# timeseries_data <- select(timeseries_data, -growthFactor)
# 
# timeseries_data$level %>% as.factor() %>% levels()
# 
...
#   filter(country == "United States") %>%
#   group_by(state) %>%
#   filter(state == "California", date == "2020-04-04", county != "NA") %>%
#   summarise(total = sum(cases, na.rm = TRUE))
